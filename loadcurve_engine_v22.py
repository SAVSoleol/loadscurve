...
ENGINE_VERSION = "2.2.0-courbe-mensuelle-U"
...
def generate(cfg: Config) -> pd.DataFrame:
    ...
    if cfg.direct_heating:
        monthly_weights = np.array([
            14.2, 12.3, 10.1, 7.8, 6.5, 5.0,
            4.3, 5.0, 6.6, 9.1, 11.8, 15.1
        ], dtype=float)
    elif cfg.heat_pump:
        monthly_weights = np.array([
            13.2, 11.7, 9.8, 7.8, 6.9, 5.5,
            4.8, 5.5, 6.9, 8.9, 10.9, 14.1
        ], dtype=float)
    else:
        # Profil résidentiel saisonnier de référence demandé :
        # hiver élevé, minimum en juillet, reprise à l'automne.
        monthly_weights = np.array([
            12.0, 10.8, 9.4, 8.0, 7.3, 6.2,
            5.5, 6.1, 7.2, 8.8, 10.3, 12.4
        ], dtype=float)
